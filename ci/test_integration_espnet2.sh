...
if [ "${task}" == "asr" ] || [ "${task}" == "all" ]; then
    ...
    for t in ${feats_types}; do
        for t2 in ${token_types}; do
            echo "::group::==== feats_type=${t}, token_types=${t2} ==="
        ...
        use_lm=false
        echo "::group::==== feats_type=raw_copy, token_types=bpe ==="
        cp -r dump/raw data/
...
if [ "${task}" == "asr_transducer" ] || [ "${task}" == "all" ]; then
    ...
    if python3 -c "from warprnnt_pytorch import RNNTLoss" &> /dev/null; then
        ...
        if [ "$(python3 -c "import torch; print(torch.cuda.is_available())")" == "True" ]; then
            echo "::group::==== Multi-Blank Transducer, feats_type=raw, token_types=bpe ==="
    ...
    if python3 -c "from warprnnt_pytorch import RNNTLoss" &> /dev/null; then
        ...
        for t in ${token_types}; do
            ...
            echo "::group::==== [Conformer-RNN-T] feats_type=raw, token_types=${t}, model_conf.extract_feats_in_collect_stats=False, normalize=utt_mvn ==="
            ...
            echo "::group::==== [Streaming Conformer-RNN-T] feats_type=raw, token_types=${t}, model_conf.extract_feats_in_collect_stats=False, normalize=utt_mvn ==="
            ./run.sh --asr_config "" --asr_task "asr_transducer" --ngpu 0 --stage 10 --stop-stage 13 --skip-packing false --feats-type "raw" --token-type ${t} \
                --feats_normalize "utterance_mvn" --python "${python}" --inference_asr_model "valid.loss.best.pth" \
