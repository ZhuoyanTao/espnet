...
dumpdir=dump/24k            # Directory to dump features.
expdir=exp_seen              # Directory to save experiments.
...
inference_model=12epoch.pth # Model path for decoding.
                                   # e.g.
...
# robust TMPDIR for torch.multiprocessing / HF / librosa etc.
if [ -n "${SLURM_JOB_ID:-}" ]; then
  export TMPDIR="${SLURM_TMPDIR:-/dev/shm/$USER/$SLURM_JOB_ID}"
else
  export TMPDIR="/work/nvme/bbjs/ttao3/tmp/${USER}-local"
fi
mkdir -p "$TMPDIR"
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            # shellcheck disable=SC2086
            utils/split_scp.pl "${key_file}" ${split_scps}
            # Decide whether to pass --model_file (only for .pth etc., not for .safetensors/F5)
            _model_arg=""
            if [ -n "${inference_model}" ] && [ "${inference_model}" != "none" ] && [ "${inference_model}" != "None" ]; then
            case "${inference_model}" in
                *.safetensors)
                _model_arg="" ;;  # F5-TTS: load via tts_conf.pretrained_ckpt in config.yaml
                *)
                if [ -f "${tts_exp}/${inference_model}" ]; then
                    _model_arg="--model_file ${tts_exp}/${inference_model}"
                else
                    log "WARNING: ${tts_exp}/${inference_model} not found; continuing without --model_file"
                    _model_arg=""
                fi
                ;;
            esac
            ...
            # shellcheck disable=SC2046,SC2086
            # --model_file "${tts_exp}"/"${inference_model}" \ #moved out for zero shot run
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/tts_inference.JOB.log \
                ${python} -m espnet2.bin.tts_inference \
                    ...
                    --key_file "${_logdir}"/keys.JOB.scp   ${_model_arg} \
...
if ! "${skip_scoring}"; then
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        log "Stage 9 GT file candidate = ${_gt_file}"
        if [ -f "${_gt_file}" ]; then
            log "✅ Found GT wav.scp: ${_gt_file}"
        else
            log "❌ GT wav.scp not found at: ${_gt_file}"
        fi
