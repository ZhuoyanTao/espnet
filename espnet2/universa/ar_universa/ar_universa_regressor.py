...
class MetricDecoderScorer(torch.nn.Module):
    def __init__(self, decoder, token_head):
        super().__init__()
        self.decoder = decoder
        self.token_head = token_head

    def forward(self, hs_pad, hs_len, ys_in_pad, ys_in_lens):
        hidden, _ = self.decoder(hs_pad, hs_len, ys_in_pad, ys_in_lens)
...
class ARUniversa(AbsUniversa):
    def __init__(
        ...
        **kwargs,
        ...
        #Terry Modification: Add regressor to predict token real values
        local_decoder_params = metric_decoder_params.copy()
        local_decoder_params["use_output_layer"] = False
        # Use the version without internal output layer -> returns hidden states
        decoder_hidden_dim = metric_decoder_params.get("adim", embedding_size)
        ...
        # Classification head: hidden -> token logits (same behavior as before)
        self.metric_token_head = torch.nn.Linear(
            decoder_hidden_dim, metric_vocab_size
        )
        
        # Regression head: hidden -> real-valued prediction
        # (1 value per token position; we'll mask which ones actually matter)
        self.metric_value_head = torch.nn.Linear(
        ...
        self.regression_criterion = torch.nn.MSELoss(reduction="sum")
    ...
    def forward(
        ...
        **kwargs,
        ...
        #modify from Terry to add real value prediction
        metric_value_token = metrics.get("metric_value_token", None)
        #end modification
        ...
        loss_ar_decoder, acc_ar_decoder, value_ar_decoder = self._calc_decoder_loss(
            audio_enc, audio_enc_lengths, metric_token, metric_token_lengths, metric_value_token,
        )
    ...
    def _calc_decoder_loss(
        ...
        metric_value_token: Optional[torch.Tensor] = None,
        ...
        # 1. Forward decoder -> hidden states (because use_output_layer=False)
        decoder_hidden, _ = self.decoder(
            audio_enc, audio_enc_lengths, ys_in_pad, ys_in_lens
        )  # (B, U+1, H)

        # 2. Classification head: hidden -> token logits
        token_logits = self.metric_token_head(decoder_hidden)  # (B, U+1, V)

        # 2a. CE loss on tokens
        loss_ce = self.ar_criterion(token_logits, ys_out_pad)

        # 2b. Token accuracies (overall + "value" tokens)
        acc_ar_decoder = th_accuracy(
            token_logits.view(-1, self.metric_vocab_size),
            ys_out_pad,
            ignore_label=self.metric_token_pad_value,
        )

        # Keep your original "value tokens are every second position" heuristic
        value_token_logits = token_logits[:, 1::2, :]     # (B, U_v, V)
        value_token_targets = ys_out_pad[:, 1::2]         # (B, U_v)

        acc_value_ar_decoder = th_accuracy(
            value_token_logits.reshape(-1, self.metric_vocab_size),
            value_token_targets,
            ignore_label=self.metric_token_pad_value,
        )

        # 3. Regression head: hidden -> real-valued prediction (optional)
        loss_reg = torch.zeros((), device=audio_enc.device)

        if metric_value_token is not None:
            # align metric_value_token with metric_token (trim to max length)
            metric_value_token = metric_value_token[:, : metric_token_lengths.max()]

            pad_value = self.metric_pad_value

            # add <sos> so shape matches ys_out_pad (B, U+1)
            value_with_sos = F.pad(
                metric_value_token, (1, 0), value=pad_value
            )

            # predict a scalar at every position
            value_pred = self.metric_value_head(decoder_hidden).squeeze(-1)  # (B, U+1)

            # only regress on positions that:
            #  - are not padding tokens
            #  - have a real value (not pad_value)
            valid_mask = (
                (ys_out_pad != self.metric_token_pad_value)
                & (value_with_sos != pad_value)
            )

            if valid_mask.any():
                pred_flat = value_pred[valid_mask]
                target_flat = value_with_sos[valid_mask]
                # average MSE over valid positions
                loss_reg = (
                    self.regression_criterion(pred_flat, target_flat)
                    / valid_mask.sum()
                )

        # 4. Total loss = CE + λ * regression
        lambda_reg = 1.0  # tune this hyperparameter
        loss_ar_decoder = loss_ce + lambda_reg * loss_reg
    ...
    def set_inference(
        ...
        self.metric_decoder_scorer = MetricDecoderScorer(self.decoder, self.metric_token_head)

        scorers = {
            "metric_decoder": self.metric_decoder_scorer,
        }
        # scorers = {
