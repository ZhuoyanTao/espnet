...
class ARUniversa(AbsUniversa):
    def __init__(
        ...
        use_hificodec_quantizer: bool = False,               # off by default
        hificodec_mode: str = "replace",                     # "replace" | "concat"
        hificodec_cfg: Optional[dict] = None,                # n_groups, n_codes, etc.
        ...
        **kwargs,
        ...
        self.use_hificodec_quantizer = use_hificodec_quantizer
        self.hificodec_mode = hificodec_mode
        self.hificodec_cfg = hificodec_cfg or {}

        
        # --- Optional HiFiCodec quantizer head (keeps baseline untouched when disabled) ---
        self.quantizer_weight = float(self.hificodec_cfg.get("weight", 0.5))
        self.quant_dim = 512  # target dim for quantizer

        # current fused encoder dim (will grow as we add ref_audio/text below)
        decoder_input_dim_base = embedding_size
        if self.use_ref_audio:
            decoder_input_dim_base += embedding_size
        if self.use_ref_text:
            decoder_input_dim_base += embedding_size

        # Prepare projection + quantizer if enabled
        if self.use_hificodec_quantizer:
            self.pre_q_proj = torch.nn.Linear(decoder_input_dim_base, self.quant_dim)
            self.hifq = HiFiCodecQuantizer(
                dim=self.hificodec_cfg.get("dim", 512),
                n_groups=self.hificodec_cfg.get("n_groups", 8),
                n_codes=self.hificodec_cfg.get("n_codes", 1024),
                codebook_loss_lambda=self.hificodec_cfg.get("codebook_loss_lambda", 1.0),
                commitment_loss_lambda=self.hificodec_cfg.get("commitment_loss_lambda", 0.25),
                residual_layers=self.hificodec_cfg.get("residual_layers", 2),
            )

        # Decide final decoder input size
        if self.use_hificodec_quantizer:
            if self.hificodec_mode == "replace":
                decoder_input_size = self.quant_dim
            elif self.hificodec_mode == "concat":
                decoder_input_size = decoder_input_dim_base + self.quant_dim
            else:
                raise ValueError(f"Unknown hificodec_mode: {self.hificodec_mode}")
        else:
            decoder_input_size = decoder_input_dim_base
        
        #end  HiFCodec quantizer block ---
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert (
            ...
            audio.shape[0], metrics.shape[0], metric_token_lengths.shape[0]
        )
        ...
        decoder_in, decoder_in_lengths, q_loss = self._maybe_quantize(
        audio_enc, audio_enc_lengths
        )
        ...
        # )
        loss_ar_decoder, acc_ar_decoder, value_ar_decoder = self._calc_decoder_loss(
            decoder_in, decoder_in_lengths, metric_token, metric_token_lengths
        )
        ...
        loss = loss_ar_decoder if q_loss is None else loss_ar_decoder + self.quantizer_weight * q_loss
        stats["loss_ar_decoder"] = loss_ar_decoder.detach()
        if q_loss is not None:
            stats["loss_q"] = q_loss.detach()
    ...
    def _maybe_quantize(self, fused: torch.Tensor, lengths: torch.Tensor):
        """
        fused: (B, T, D_fused)  -> returns (decoder_in, lengths, q_loss or None)
        """
        if not self.use_hificodec_quantizer:
            return fused, lengths, None

        # project to 512, run HiFiCodecQuantizer on (B, 512, T)
        z = self.pre_q_proj(fused)           # (B,T,512)
        z = z.transpose(1, 2)                # (B,512,T)
        z_q, q_loss, _ = self.hifq(z)        # z_q: (B,512,T)
        z_q = z_q.transpose(1, 2)            # (B,T,512)

        if self.hificodec_mode == "replace":
            decoder_in = z_q
        elif self.hificodec_mode == "concat":
            decoder_in = torch.cat([fused, z_q], dim=-1)
        else:
            raise ValueError(f"Unknown hificodec_mode: {self.hificodec_mode}")

        return decoder_in, lengths, q_loss
    ...
    def inference(
        ...
        **kwargs,
        ...
        # Optionally quantize (no-op when disabled)
        decoder_in, decoder_in_lengths, _ = self._maybe_quantize(
            audio_enc, audio_enc_lengths
        )
        ...
        nbest_hyps = self.search_module.forward(decoder_in[0])
        ...
        pred_metrics["encoded_feat"] = decoder_in
        return pred_metrics
