...
class MetricTokenizer(AbsMetricTokenizer):
    def __init__(
        ...
        # DEBUG: show available metrics and some vocab
        logger.info(
            "MetricTokenizer initialized. Available metric names (offset keys): %s",
            list(self.metric_offset.keys()),
        )
        logger.info(
            "MetricTokenizer tokenize_metric list: %s",
            self.tokenize_metric,
        )
    ...
    def get_metric_meta_label(self, metric_name: str) -> int:
        ...
        if meta_label_index is None:
            logger.error(
                "get_metric_meta_label: asked for %s, token %s not found. "
                "Available metric_offset keys: %s",
                metric_name,
                meta_label_token,
                list(self.metric_offset.keys()),
            )
            logger.error(
                "Example meta-label tokens: %s",
                [t for t in self.vocab if t.endswith("@meta_label")][:50],
            )
